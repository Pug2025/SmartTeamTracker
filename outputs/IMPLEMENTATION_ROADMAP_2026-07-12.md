...
|---|---|---|---|
| 0 | Hardening: data-integrity, trust, install | none — start now | ~2 days |
| 1 | Ice spectator reskin (payload prep + skin) | Phase 0 complete | ~2 days |
| 2 | Ice token system app-wide (quiet variant) | Phase 1 shipped + 48h soak | ~3–4 days |
...
Apply tokens WITHOUT atmosphere: solid `--surface-*` fills (no photo, no blur, no blend modes on the coach path); nesting communicates via surface steps (modal `--surface-1` → section `--surface-2` → tile `--surface-3`) instead of same-elevation hairline cards. Live-game THEM/US zone tints re-derived from `--them-tint`/`--us-tint`. Summary rings/bars recolored to semantic tokens. Move the summary's **Delete Game** button out of the final position into a small overflow row ("⋯" or de-emphasized text link) — the money screen must not end on red.

### P2.5 Broadcast surfaces
Landing + auth restyled with tokens; landing gets the frost texture treatment and a 2-column ≥900px hero (copy left, phone mockup right); post-game summary gets ONE restrained ice-texture header band behind the score headline (no full-photo background — it's a data screen).
...
- **P3.1 Wordmark/lockup** → header (replaces Saira placeholder), landing hero, auth screen, spectator footer, share-card template.
- **P3.2 App icons** → replace P0.8 placeholders (`icon-192/512/maskable/apple-touch/favicon`); regenerate `favicon.ico`; bump SW.
- **P3.3 Landing hero image** → desktop landing left-column art; compress to WebP ≤ 300 KB, `loading="lazy"` below fold.
- **P3.4 Share/OG art** → `spectator-share.html` preview, og:image 1200×630 for landing and spectator, and the Phase 6 share-card generator background.
...
| 5 | `share-card-bg-story.png` + `share-card-bg-og.png` | 1080×1920 and 1200×630 ice-texture frames with reserved center zone (~70%) for live scoreboard/crest overlay; subtle wordmark placement bottom. | post-game share cards, og:image |
...
| 7 | `texture-frost-tile.png` (optional) | ~512px seamless frost/noise tile — replaces the live fractalNoise blend layer if the perf gate in P1.3(c) fails. | spectator/landing backgrounds |
